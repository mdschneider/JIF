...
class TestRoaster(unittest.TestCase):
    ...
    def test_init_gal_prior(self):
        rstr = jif.Roaster(lnprior_omega=None)
        self.assertEqual(rstr.lnprior_omega, jif.RoasterModule.EmptyPrior())
...
#     roaster.set_param_by_name('psf_fwhm_1', 0.8)
#     roaster.set_param_by_name('psf_e_1', 0.01)
#     roaster.set_param_by_name('psf_fwhm_2', 0.7)
#     roaster.set_param_by_name('psf_e_2', 0.02)
#     #
...
#     assert np.isclose(p[6], 0.7)
#     assert np.isclose(p[7], 0.02)
#     return None
