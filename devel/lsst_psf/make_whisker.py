...
def get_whisker_map(psf, xx, yy):
    ...
    for x, y in zip(xx, yy):
        ...
        csq = np.sqrt((Ixx - Iyy)**2 + 4*Ixy**2) * pixel_scale_arcmin ** 2
...
def plot_whisker(xx, yy, e1, e2, j_pupil=1, j_field=1):
    ...
    plt.quiverkey(Q, 1.3, 0.8, arrow_scale*0.2, '0.2 arcsec', coordinates='inches')
    # plt.show()
    plt.savefig("whisker_images/whisker_image_{:d}_{:d}.png".format(j_pupil, j_field))
    plt.close()
...
if __name__ == '__main__':
    ...
    for j_pupil in xrange(5, 7):
        for j_field in xrange(5, 7):
            ...
            plt.savefig("aberration_images/aberr_{:d}_{:d}.png".format(j_pupil, j_field))
            plt.close()
